...
// 16000000 /* prefer_speed */   Original  value
...
-gfx 800x480 rotaion 1 és ROTATION_RIGHT 90fokban elfordítja a kijelzőt (fél kijelző) lv_display_t *disp = lv_display_create(480, 800); teljes kijelző! [USB_DOWN]
-gfx 800x480 rotaion 2 és ROTATION_NORMAL 180fokban elfordítja a kijelzőt lv_display_t *disp = lv_display_create(800, 480); teljes kijelző! [USB_RIGHT]
-gfx 800x480 rotaion 3 és ROTATION_LEFT 270fokban elfordítja a kijelzőt  lv_display_t *disp = lv_display_create(480, 800); teljes kijelző! [USB_UP]
*/
...
void go_touch() {
  ...
  if (!scr_touch) {
    ...
    x_lbl = lv_label_create(scr_touch);
    lv_obj_set_style_text_font(x_lbl, &lv_font_montserrat_18, LV_PART_MAIN);
    lv_obj_set_style_text_color(x_lbl, lv_color_hex(0x000000), LV_PART_MAIN);
    lv_obj_align(x_lbl, LV_ALIGN_TOP_LEFT, 20, 20);  // bal felső sarokba pl.

    // callback
    lv_obj_add_event_cb(
      scr_touch, [](lv_event_t *e) -> void {
        if (lv_event_get_code(e) == LV_EVENT_PRESSED || lv_event_get_code(e) == LV_EVENT_PRESSING) {
          ...
          lv_label_set_text(x_lbl, "");  // vagy "Touch released"
        }
      },
      LV_EVENT_ALL, NULL);
  }
